...
//computes the prediction and the alternate prediction
    ...
    tage_pred_confidence = gtable[HitBank][GI[HitBank]].ctr;
...
//USE_ALT_ON_NA is positive  use the alternate prediction
  ...
  int8_t predict_brindirect (uint64_t pc, uint64_t* pred_taken)
  {
    ...
    //if (isIndBr){ // computes the table addresses and the partial tags
    if(true){
		for (int i = 0; i <= NHIST; i++) {
...
//  UPDATE  FETCH HISTORIES
    ...
    void HistoryUpdate (uint32_t pc, InstClass insn, bool taken,
			uint64_t target,  int &Y, folded_history * H,
			folded_history * G, folded_history * J)
    {
      bool isCondBr = insn == condBranchInstClass;
      bool isIndBr = insn == uncondIndirectBranchInstClass;
      bool isCallBr = insn == (uncondDirectBranchInstClass || uncondIndirectBranchInstClass);
      int maxt = (isIndBr) ? 10 : 0;
      if (!(isIndBr))     if (isCallBr)
	maxt = 5;
...
// PREDICTOR UPDATE
    ...
    void update_brindirect (uint32_t pc, InstClass insn, bool taken,
			    uint64_t target)
    {
      //bool isIndBr = insn == uncondIndirectBranchInstClass;
      int NRAND = MYRANDOM ();
