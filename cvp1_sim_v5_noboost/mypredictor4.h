...
class gentry{			// value prediction global table entry
	public:
  	...
  	uint64_t target;		//64 bit predicted value
  	int8_t u;			//1 bit
    gentry(){
    	ctr = 0;
    	tag = 0;
...
class my_predictor{
  ...
  // tag computation
  uint16_t gtag(uint64_t pc, int bank, folded_history * ch0,
		 folded_history * ch1){
  ...
  int MYRANDOM(){
    ...
    taken_ctr = gtable[HitBank][GI[HitBank]].ctr;
    ...
    if(AltBank >= 0)
      if((USE_ALT_ON_NA >= 0) & (gtable[HitBank][GI[HitBank]].ctr == 0)){
      	//fprintf(stdout,"alt is %" PRIx64 "\t",alttaken);
      	//fprintf(stdout,"taken is %" PRIx64 "\n",tage_pred);
				tage_pred = alttaken;
				taken_ctr = gtable[AltBank][GI[AltBank]].ctr;
			}
  }

	// PREDICTION
  int8_t predict_brindirect(uint64_t pc, uint64_t* pred_taken){
  	int8_t confidence = 0;
  	uint64_t temp;
  ...
  void FetchHistoryUpdate(uint64_t pc, InstClass insn, bool taken,
		uint64_t target){
  	if(insn == 0){
	...
	void HistoryUpdate(uint64_t pc, InstClass insn, bool taken,
      ...
      //if(!(insn == uncondIndirectBranchInstClass))
      //	if(insn == uncondIndirectBranchInstClass) //call somehow
			//		maxt = 5;
	...
	// PREDICTOR UPDATE
  void update_brindirect(uint64_t pc, InstClass insn, bool taken,
		uint64_t target){
    int NRAND = MYRANDOM();
    bool flag = insn == 0;
    if(flag){
  ...
  HistoryUpdate(pc, insn, taken, target,  Retire_ptghist,
