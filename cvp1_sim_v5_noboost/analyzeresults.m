...
data = data(ind,:);
data = 100-data;
compute_fp = data(1:15,:);
compute_int = data(16:58,:);
srv = data(59:end,:);
srvsize = size(srv,1);
figarray = zeros(1,7);
axarray = zeros(1,7);
...
axarray(1) = gca;
bar(ind,compute_fp);
mx = max(max(compute_fp));
mn = min(min(compute_fp));
ylim([mn*.9 mx*1.1]);
title('compute\_fp traces');

figure(figarray(2));
ind = 16:37;
axarray(2) = gca;
bar(ind,compute_int(1:22,:));
mx = max(max(compute_int(1:22,:)));
mn = min(min(compute_int(1:22,:)));
ylim([mn*.9 mx*1.1]);
title('compute\_int traces 1');

figure(figarray(3));
ind = 38:58;
axarray(3) = gca;
bar(ind,compute_int(23:43,:));
mx = max(max(compute_int(23:43,:)));
mn = min(min(compute_int(23:43,:)));
ylim([mn*.9 mx*1.1]);
title('compute\_int traces 2');
...
axarray(5) = gca;
bar(ind,srv(21:40,:));
mx = max(max(srv(21:40,:)));
mn = min(min(srv(21:40,:)));
ylim([mn*.9 mx*1.1]);
title('srv traces 2');
...
axarray(6) = gca;
bar(ind,srv(41:60,:));
mx = max(max(srv(41:60,:)));
mn = min(min(srv(41:60,:)));
ylim([mn*.9 mx*1.1]);
title('srv traces 3');
...
axarray(7) = gca;
bar(ind,srv(61:76,:));
mx = max(max(srv(61:76,:)));
mn = min(min(srv(61:76,:)));
ylim([mn*.9 mx*1.1]);
title('srv traces 4');
...
for i=1:7
  figure(figarray(i));
  axes(axarray(i));
  legend({'base predictor','myvtage'});
  ylabel('percent correct predictions');
  
end
avg = sum(data)/size(data,1);


%traces = {'tmp433.milc-1-3243-00017.ckpt.23117',...
