...
public:
  ...
  int gindex(uint64_t seq_no, uint64_t pc, int bank, folded_history * ch_i){
    int index;
    index =
    	seq_no ^ (seq_no >> (abs (logg[bank] - bank) + 1)) ^
      ch_i[bank].comp;
  ...
  uint16_t gtag(uint64_t seq_no, uint64_t pc, int bank, folded_history * ch0,
		 folded_history * ch1){
  	int tag = seq_no ^ ch0[bank].comp ^ (ch1[bank].comp << 1);
...
// to allocate entries  in the loop predictor
  ...
  int8_t getPrediction(uint64_t seq_no, uint64_t pc, uint8_t piece, uint64_t* prediction){
  	...
  	#ifdef DEBUG3
  	if(alttaken == 0)
  		fprintf(stdout,"gseq = %" PRIx64 "\tprediction = %" PRIx64 " from hit [%u,%u]\n",
							seq_no,tage_pred, HitBank, GI[HitBank]);
		else
  		fprintf(stdout,"gseq = %" PRIx64 "\tprediction = %" PRIx64 " from alt [%u,%u]\n",
				seq_no,tage_pred, AltBank, GI[AltBank]);
		fprintf(stdout, "USE_ALT_ON_NA = %d\n", USE_ALT_ON_NA);
		#endif
		return conf;
  }
	...
	void speculativeHistoryUpdate(uint64_t seq_no, uint64_t pc, InstClass insn, bool taken,
		...
		#ifdef DEBUG4
		fprintf(stdout,"pred_result = %u\n", prediction_result);
		#endif
		if(last_pc != pc){
			last_insn = insn;
			last_taken = taken;
			last_pc = pc;
			HistoryUpdate(seq_no, pc, insn, taken, target, Fetch_ptghist,
		     Fetch_ch_i, Fetch_ch_t[0], Fetch_ch_t[1]);
		}
  }
	void HistoryUpdate(uint64_t seq_no, uint64_t pc, InstClass insn, bool taken,
  ...
  bool update_predictor(uint64_t seq_no, uint64_t target){
  	...
  	//if(tage_pred == target){
			fprintf(stdout,"useq = %" PRIx64 "\tupdate = %" PRIx64 "\tactual = %" PRIx64 "\n",
							seq_no,tage_pred, target);
		//}
  	#endif
  	#ifdef DEBUG6
  	fprintf(stdout,"Longestmatch = %" PRIx64 " target = %" PRIx64 "\n",LongestMatchPred, target);
  	#endif
  	...
  	HistoryUpdate(seq_no, last_pc, last_insn, last_taken, target,  Retire_ptghist,
