...
bool getPrediction(uint64_t seq_no, uint64_t pc, uint8_t piece, uint64_t& predicted_value){
  ...
  //mypred->gindex(pc,  );
  int8_t confidence = mypred->predict_brindirect(pc, &predicted_value);
  //printf("confidence = %d\n", confidence);
  printf("prediction = %" PRIx64 "\t",predicted_value);
  return true;
  return confidence>=1;
}
...
void speculativeUpdate(uint64_t seq_no,    		// dynamic micro-instruction # (starts at 0 and increments indefinitely)
		       ...
		       uint64_t dst) {
	stats[insn]++;
	//mypred->FetchHistoryUpdate(pc,insn,prediction_result == 1,src1);
	//mypred->update_brindirect(pc,insn,prediction_result == 1,src1);
...
void updatePredictor(uint64_t seq_no,		// dynamic micro-instruction #
		...
		//mypred->FetchHistoryUpdate(pc,insn,prediction_result == 1, piece);
	//printf("actual value is %" PRIx64 "\n", actual_value);
	mypred->update_brindirect(		
}
